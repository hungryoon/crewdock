...
- `CREW_PROJECT`(여기선 `synt`)와 게이트웨이 포트(`CREW_GATEWAY_HTTPS_PORT`=443,
  `CREW_ROUTER_PORT`=9400, `CREW_AUTH_PORT`=9401)는 `_shared.env`에 들어가며 **`crew init`이
  한 번 써주면 끝**이다. 이후 명령들은 이 값을 읽어 동작한다.
- 모든 Docker 객체는 프로젝트 이름으로 프리픽스된다: 인스턴스 컨테이너는 `<project>-<instance>`
  (예: `synt-alice`), 게이트웨이 컨테이너는 `<project>-gateway-{router,auth,broker}`.
...
`crew init`이 `_shared.env`를 이미 만들고 `CREW_PROJECT`/포트/쿠키 시크릿/OAuth 자리표시자를
써뒀다(루트의 `instances/_shared.env`, chmod 600). 여기서 OAuth 값만 채운다:
```
