...
```
- 이 인스턴스를 볼 수 있는 계정 목록. **비어 있으면 expose가 거부된다(fail-closed).**
...
| 작업 | 명령 |
|---|---|
| 화이트리스트 변경 | `instances/<name>/instance.env`의 `CREW_ALLOWED_EMAILS` 수정 → `crew expose <name>` (게이트웨이 allowlist 갱신) |
| 사용자 접근 회수 | `crew unexpose <name>` (게이트웨이에서 제거; 데이터는 유지) |
...
- **인스턴스별 이메일 화이트리스트** — 라우터가 요청마다 그 인스턴스의 `CREW_ALLOWED_EMAILS`를
  확인. 목록에 없으면 403. 인덱스에도 허용된 인스턴스만 노출.
...
- **`crew expose`가 "CREW_ALLOWED_EMAILS is empty"** → 그 인스턴스 `instance.env`에 화이트리스트 미설정. ①② 참고.
- **로그인은 되는데 인스턴스가 안 보임** → 그 구글 계정이 화이트리스트에 없음. 수정 후 `crew expose <name>`.
...
## 5. 한눈에 (사용자 1명 추가)
```sh
crew create bob --credential anthropic          # 인스턴스 + 모델용 API 키
